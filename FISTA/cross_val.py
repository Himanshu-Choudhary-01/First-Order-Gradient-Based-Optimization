...
def cross_val(penalty, lambda_, n_folds, K, y, n_iter=1000):
    ...
    infos, mean, std, auc_mean = _compute_info(estimator, K, y, lambda_, folds)
    result = Bunch()
    result['infos'] = infos
    result['penalty'] = penalty
    result['lambdas'] = lambda_
    result['std_score'] = std
    result['mean_score'] = mean
    result['auc_mean_score'] = auc_mean
    result['n_folds'] = n_folds

    return result


def intern_cross_val(estimator, lambdas, n_folds_int, n_samples, n_kernels,
        K, y, train_lines, train_columns, test):
